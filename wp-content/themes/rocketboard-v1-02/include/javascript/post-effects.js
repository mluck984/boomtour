...
jQuery(document).ready(function(){ 
	...
	//Thumbnail Image Type
	jQuery('#post-option-board-image-link-type').change(function(){
		var choose_url = jQuery(this).parents("#board-image-link").find("#post-option-board-image-url");
		if(jQuery(this).val() == "Link to Current Post" || jQuery(this).val() == "Lightbox to Current Thumbnail"){
			choose_url.parents(".meta-body").slideUp();
		}else{
			choose_url.parents(".meta-body").slideDown();
	...
	jQuery('#post-option-board-image-link-type').each(function(){
		var choose_url = jQuery(this).parents("#board-image-link").find("#post-option-board-image-url");
		if(jQuery(this).val() == "Link to Current Post" || jQuery(this).val() == "Lightbox to Current Thumbnail"){
			choose_url.parents(".meta-body").css('display','none');
		}else{
	...
	jQuery('#post-option-board-thumbnail-type').change(function(){
		var parent_node = jQuery(this).parents(".meta-body");
		if(jQuery(this).val() == "Feature Image" ){
			parent_node.siblings('#board-color').slideUp();
			parent_node.siblings('#board-feature-image').slideDown();
		}else{
			parent_node.siblings('#board-color').slideDown();
			parent_node.siblings('#board-feature-image').slideUp();
		}
	});
	jQuery('#post-option-board-thumbnail-type').each(function(){
		var parent_node = jQuery(this).parents(".meta-body");
		if(jQuery(this).val() == "Feature Image" ){
			parent_node.siblings('#board-color').css('display', 'none');
			parent_node.siblings('#board-feature-image').css('display', 'block');
		}else{
			parent_node.siblings('#board-color').css('display', 'block');
			parent_node.siblings('#board-feature-image').css('display', 'none');
		}
	});	
	
	//Board Thumbnail Icon
	jQuery('#post-option-color-board-info').change(function(){
		var parent_node = jQuery(this).parents(".meta-body");
		if(jQuery(this).val() == "Icon" ){
			parent_node.siblings('#board-thumbnail-icon').slideDown();
		}else{
			parent_node.siblings('#board-thumbnail-icon').slideUp();
		}
	});
	jQuery('#post-option-color-board-info').each(function(){
		var parent_node = jQuery(this).parents(".meta-body");
		if(jQuery(this).val() == "Icon" ){
			parent_node.siblings('#board-thumbnail-icon').css('display', 'block');
		}else{
			parent_node.siblings('#board-thumbnail-icon').css('display', 'none');
		}
	});		
