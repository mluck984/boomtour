<?php get_header(); 
	...
	<div class="page-wrapper single-blog <?php echo $sidebar_array['sidebar_class']; ?>">
		<?php
			...
			print_page_header(get_the_title(), $single_caption);
			...
			if ( have_posts() ){
				while (have_posts()){
					...
					echo '<div class="blog-date-wrapper">' . get_the_time($gdl_date_format) . '</div>';	
					...
					$portfolio_tag = get_the_term_list( $post->ID, 'portfolio-tag', '', ', ' , '' );
					if(!empty($portfolio_tag)){
						echo '<div class="blog-tag">' . $portfolio_tag . '</div>';
					}
